...
class App(widgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("with Andrey Orlyanskiy")
        ...
        self.ui.capture_image.clicked.connect(self.on_capture_image_click)
    ...
    def on_save_image_click(self):
        ...
        if selected_file_path is not None and self.converted_image is not None:
            self.converted_image.save(os.path.join(selected_file_path, f'sr_{self.selected_image_name}.png'))
    ...
    def perform_inference(self):
        ...
        self.ui.super_resolve.setText("Upgrade Image")
        self.setWindowTitle("with Andrey Orlyanskiy")
        self.ui.super_resolve.repaint()

    def on_capture_image_click(self):
        if self.camera is None:
            self.camera = cv2.VideoCapture(0)  # Используем камеру с индексом 0
       ...
       # "Прогреваем" камеру, чтобы снимок не был тёмным
        ...
        if ret:
            ...
            self.stop_camera()  # Остановка камеры
